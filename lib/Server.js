...
function Server(compiler, options) {
	...
	app.get("/webpack-dev-server", function(req, res, next) {
		res.setHeader("Content-Type", "text/html");
...
Server.prototype.serveMagicHtml = function(req, res, next) {
	var _path = req.path === '/' ? '/bundle' : req.path;
	try {
		if(!this.middleware.fileSystem.statSync(this.middleware.getFilenameFromUrl(_path + ".js")).isFile())
			return next();
		if (process.env.WEBPACK_SERVER_HTML) {
			var payload = '';
			payload += '<script src="/webpack-dev-server.js"></script>';
			payload += '<script src="' + _path + '.js' + (req._parsedUrl.search || '') + '"></script>';
			res.end(fs.readFileSync(process.env.WEBPACK_SERVER_HTML).toString().replace(/\<\/body\>/, payload + '</body>'));
		}
		else {
